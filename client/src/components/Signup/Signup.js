...
const Signup = () => {
	...
	const handleSubmit = async (e) => {
		...
		const {error, message} = await signupSuccess.json();
		if (error) {
			alert(message);
		} else {
			if(message) {
					...
					<div className="form-box offset-md-3 col-md-6">
						...
						<form method="post">
							<div className="form-group">
								<label>Name:</label>
								<input type="text" name="name" placeholder="Enter Full Name..." value={name} className="form-control" onChange={handleValueChange} />
							</div>
							<div className="form-group">
								<label>Email:</label>
